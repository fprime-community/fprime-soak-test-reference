 ...
 *   4. Configure components requiring custom configuration (includes prmDb file path)
 *   5. Call the autocoded `readParameters()` function to load PrmDb.dat into the parameter database
 ...
 *
 * Step 4 and step 8 are custom and supplied by the project. The ordering of steps 1–7 is critical for
 * F´ topologies to function. Configuration (step 4) must precede readParameters so prmDb knows which file
 * to open; readParameters must precede loadParameters so components see persisted values. Step 8 may come
 * before or after the active component initializations. Since these custom tasks often start radio
